...
//
// For Next Release (1.2.0):
// * wxPython: SetPropertyAttribute. Type mapper for wxVariant?
// * Fix auto-center when no scrollbar in the first place.
// * ComboBox Editor doesn't handle Esc and Tab right.
// * Maybe: Dir dlg message attribute for wxDirProperty.
// * Maybe: Dir dlg title attribute for wxDirProperty.
//
// Maybe later:
...
void wxPropertyGrid::SetPropertyValueString( wxPGId id, const wxString& value )
{
    wxPGProperty* p = wxPGIdToPtr(id);
    wxASSERT(p);
    if ( p && m_pState->SetPropertyValueString(p,value) )
        DrawItemAndChildren ( p );
}
