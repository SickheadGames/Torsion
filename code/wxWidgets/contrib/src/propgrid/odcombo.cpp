...
// Popupwin is really not supported on wxMAC or wxWINCE, regardless what the
// wxUSE_POPUPWIN says.
...
{
    //wxLogDebug(wxT("wxComboPopupWindow::OnKeyEvent(%i)"),(int)event.GetEventType());
...
wxPGCount wxComboPopupInterface::GetCount() const
{
    return 0;
}
...
public:
    ...
    virtual wxPGCount GetCount() const;
...
{
    // Move selection to cursor if it is inside the vlistbox.
    wxPoint pos = event.GetPosition();
    wxSize sz = GetClientSize();

    if ( pos.x >= 0 && pos.y >= 0 && pos.x < sz.x && pos.y < sz.y )
    {
        int itemHere = GetItemAtPosition(pos);
        if ( itemHere >= 0 )
        {
...
wxPGCount wxVListBoxComboInterface::GetCount() const
{
...
void wxPGOwnerDrawnComboBox::Delete(int n)
{
    if ( m_hasIntValue )
    {
        wxCHECK_RET( (n >= 0) && (n < (int)GetCount()), _T("invalid index in wxPGOwnerDrawnComboBox::Delete") );
...
wxPGCount wxPGOwnerDrawnComboBox::GetCount() const
{
    wxASSERT( m_popupInterface );
...
{
    wxCHECK_MSG( (n >= 0) && (n < (int)GetCount()), wxEmptyString, _T("invalid index in wxPGOwnerDrawnComboBox::GetString") );
    return m_popupInterface->GetString(n);
}

void wxPGOwnerDrawnComboBox::SetString(int n, const wxString& s)
{
    wxCHECK_RET( (n >= 0) && (n < (int)GetCount()), _T("invalid index in wxPGOwnerDrawnComboBox::SetString") );
...
int wxPGOwnerDrawnComboBox::DoAppend(const wxString& item)
{
    ...
    int pos = (int)m_popupInterface->GetCount();
...
int wxPGOwnerDrawnComboBox::DoInsert(const wxString& item, int pos)
{
    ...
    wxCHECK_MSG((pos>=0) && (pos<=(int)GetCount()), -1, wxT("invalid index"));
