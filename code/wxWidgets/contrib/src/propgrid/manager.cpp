...
#define wxPG_MAN_PROPGRID_FORCED_FLAGS ( wxSTATIC_BORDER | \
...
void wxPropertyGridManager::SetPropertyAttributeAll ( int attrid, wxVariant value )
{
    ...
    for ( i=0; i<m_arrPages.GetCount(); i++ )
    {
        ...
        DoSetPropertyAttribute(wxPGIdGen(page->m_state.m_properties),attrid,value,wxPG_RECURSE);
    }
}
...
{
    return ((wxPropertyGridPageData*)m_arrPages.Item(index))->m_state.m_properties;
}
                                              ...
                                              bool desc_too )
{
    ...
    // TOM: Totally broke cross platform stuff here
    // to make it look good with XP themese.
    /*
    if ( wxUxThemeEngine::Get() && wxUxThemeEngine::Get()->IsAppThemed() )
    {
      COLORREF color = wxUxThemeEngine::Get()->GetThemeSysColor( NULL, 3801 ); // border color.
      wxColour bdcol( GetRValue( color ), GetGValue( color ), GetBValue( color ) );
      dc.SetPen( bdcol );
