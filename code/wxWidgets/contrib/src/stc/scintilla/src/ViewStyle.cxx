...
void ViewStyle::Refresh(Surface &surface) {
   ...
   // TOM: We don't use STYLE_DEFAULT for ascent/decent info here,
   // but let it get set within the loop below.
   maxAscent = 0;
	maxDescent = 0;
	someStylesProtected = false;
	for (unsigned int i=0;i<(sizeof(styles)/sizeof(styles[0]));i++) {
		if (i != STYLE_DEFAULT) {
         ...
         // TOM: Everything after STYLE_DEFAULT is GUI stuff
         // which we don't use for min/max ascent info.
         if ( i < STYLE_DEFAULT )
         {
   ...
   // TOM: This used to use STYLE_DEFAULT, but that normally
   // has a font used for GUI stuff and isn't right... 0 is
