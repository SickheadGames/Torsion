...
wxFileType *wxMimeTypesManagerImpl::Associate(const wxFileTypeInfo& ftInfo)
{
    ...
    // start by setting the HKCR\\.ext entri    es
...
bool wxFileTypeImpl::AddToOpenWithList(const wxString& name)
{
    wxCHECK_MSG( !name.empty(), false, _T("AddToOpenWithList() needs name") );
    wxCHECK_MSG( !m_ext.empty(), false, _T("AddToOpenWithList() needs extension") );
    wxCHECK_MSG( !m_strFileType.empty(), false, _T("File key not found") );

    if ( !EnsureExtKeyExists() )
        return false;

    wxRegKey rkey(wxRegKey::HKCR, m_ext + _T("\\OpenWithList"));
    wxRegKey namekey(wxRegKey::HKCR, m_ext + _T("\\OpenWithList\\") + name);
    return rkey.Create() && namekey.Create();
}
