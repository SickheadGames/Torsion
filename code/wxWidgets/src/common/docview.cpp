...
wxDocument* wxDocManager::FindDocument(const wxString &path) const
{
    for (size_t i = 0; i < GetDocuments().GetCount(); ++i)
    {
        ...
        if (path.CmpNoCase(currentDoc->GetFilename()) == 0)
#else
        if (path.Cmp(currentDoc->GetFilename()) == 0)
#endif
        {
            return currentDoc;
        }
...
int wxDocManager::FindDocuments(wxDocTemplate *temp, wxList &docs ) const
{
    for (size_t i = 0; i < GetDocuments().GetCount(); ++i)
    {
        wxDocument* currentDoc = (wxDocument*)(GetDocuments().Item(i)->GetData());
        if (currentDoc->GetDocumentTemplate() == temp)
        {
...
wxDocument *wxDocManager::CreateDocument(const wxString& path, long flags)
{
    ...
    if (temp)
    {
        ...
        {
            wxDocument* currentDoc = FindDocument( path2 );
            if (currentDoc && currentDoc->GetFirstView())
            {
