...
//   gClientSceneGraph->addObjectToScene(gDecalManager);
   ...
   script[size] = 0;
   Con::evaluate(script, false, mainCS); 
   delete[] script;
   return true;
}
...
//   Memory::setBreakAlloc(104717);
   ...
   while(Game->isRunning())
   {
      ...
      if(lastFrame == -1)
      {
         lastFrameTime = Platform::getRealMilliseconds();
         lastFrame = gFrameCount;
      }

      if(gFrameCount != lastFrame)
      {
         U32 delta = Platform::getRealMilliseconds() - lastFrameTime;

         if(delta > 30)
            Con::printf("   frame = %d, time = %d", gFrameCount, delta);
         ...
         if(gFrameCount == 6300)
         {
            gProfiler->dumpToFile("autoprofile.log");
         }
      }
      */
